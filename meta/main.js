...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: "https://github.com/k1mittal/portfolio/commit/" + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
  ...
  const avgFileLength = d3.mean(commits, (d) => d.totalLines);
  dl.append("dt").text("Average file length (lines):");
  dl.append("dd").text(Math.round(avgFileLength * 100) / 100);

  const avgLineLength = d3.mean(data, (d) => d.length);
  dl.append("dt").text("Average line length (characters):");
  dl.append("dd").text(Math.round(avgLineLength * 100) / 100);
}
...
function renderScatterPlot(data, commits) {
  ...
  const rScale = d3.scaleSqrt().domain([minLines, maxLines]).range([3, 35]);
  dots
    .selectAll("circle")
